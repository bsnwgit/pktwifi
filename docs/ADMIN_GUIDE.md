...
`PKTWIFI_REMOVE_EXISTING=1` (or `0`) to answer that prompt from a script;
...
encryption key), `pktwifi.db` and its `-wal`/`-shm`, `logs/`, `backups/` and
